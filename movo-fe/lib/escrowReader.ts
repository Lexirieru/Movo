...
import { EscrowInfo, EscrowRoomDetails, ReceiverDetails } from './smartContract';
...
export const getEscrowDetailsFromContract = async (
  ...
  try {
    ...
    const escrowDetails = await contract.read.getEscrowDetails([escrowId as `0x${string}`]);
    
    // Get receiver addresses
    const receiverAddresses = await contract.read.getEscrowReceivers([escrowId as `0x${string}`]);
    
    // Get receiver details for each address
    const receivers: ReceiverDetails[] = [];
    for (const receiverAddress of receiverAddresses) {
      const receiverDetails = await contract.read.getReceiverDetails([escrowId as `0x${string}`, receiverAddress]);
      receivers.push({
        receiverAddress,
...
export const getSenderEscrows = async (
  ...
  try {
    ...
    const escrowIds = await contract.read.getUserEscrows([senderAddress as `0x${string}`]);
...
export const getReceiverEscrows = async (
  ...
  try {
    ...
    const escrowIds = await contract.read.getReceiverEscrows([receiverAddress as `0x${string}`]);
    return escrowIds.map(id => id);
  } catch (error) {
...
export const canReceiverClaim = async (
  ...
  try {
    ...
    const balanceInfo = await contract.read.getEscrowBalance([escrowId as `0x${string}`]);
    
    // Check if escrow has available balance
    if (balanceInfo[1] <= 0) { // availableBalance
      return { canClaim: false, claimableAmount: BigInt(0), reason: 'No funds available in escrow' };
    }

    // Get receiver details
    const receiverDetails = await contract.read.getReceiverDetails([escrowId as `0x${string}`, receiverAddress as `0x${string}`]);
    
    // Check if receiver is active
    if (!receiverDetails[2]) { // isActive
      return { canClaim: false, claimableAmount: BigInt(0), reason: 'Receiver is not active' };
    }
    ...
    if (claimableAmount <= 0) {
      return { canClaim: false, claimableAmount: BigInt(0), reason: 'No funds available to claim' };
    }

    // Check if escrow has sufficient available balance
    if (balanceInfo[1] < claimableAmount) { // availableBalance
      return { canClaim: false, claimableAmount: BigInt(0), reason: 'Insufficient escrow balance' };
    }

    return { canClaim: true, claimableAmount, reason: 'Can claim' };
  } catch (error) {
    console.error('Error checking if receiver can claim:', error);
    return { canClaim: false, claimableAmount: BigInt(0), reason: 'Error checking claim status' };
  }
};
...
export const getWithdrawableAmount = async (
  ...
  try {
    ...
    const withdrawableAmount = await contract.read.getWithdrawableAmount([escrowId as `0x${string}`, receiverAddress as `0x${string}`]);
