    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-semibold text-white">Group ↔ Escrow Link</h3>
          <button
            onClick={loadEscrowData}
          ...
          <div className="space-y-3">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-white font-mono text-sm break-all">{groupId}</p>
              </div>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-white font-mono text-sm break-all">{escrowData.escrowId}</p>
              </div>
            </div>
            
            <div className="flex items-center space-x-3">
              <div className="w-5 h-5 rounded-full flex items-center justify-center">
                <span className="text-lg">{escrowData.tokenType === 'USDC' ? '💵' : '🔗'}</span>
              </div>
              <div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-6">
        <div className="text-center">
          ...
          <p className="text-white/60 text-sm">
            This group is now linked to an escrow on the blockchain. 
            All escrow data can be read directly from the smart contract.
          </p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-cyan-500/10 to-blue-500/10 border border-cyan-500/20 rounded-xl p-6">
        <h4 className="text-lg font-semibold text-white mb-4">Escrow Details</h4>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-3">
            <div>
              <p className="text-white/60 text-sm">Sender Address</p>
              <p className="text-white font-mono text-sm">{formatAddress(escrowData.senderAddress)}</p>
            </div>
            ...
            <div>
              <p className="text-white/60 text-sm">Status</p>
              <p className={`font-medium ${escrowData.status === 'active' ? 'text-green-400' : 'text-yellow-400'}`}>
                {escrowData.status}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="text-white font-mono text-sm break-all">{formatAddress(escrowData.transactionHash)}</p>
            </div>
            
            <div>
              <p className="text-white/60 text-sm">Created At</p>
              <p className="text-white text-sm">{new Date(escrowData.createdAt).toLocaleString()}</p>
            </div>
            
            <div>
              <p className="text-white/60 text-sm">Receivers</p>
              <p className="text-white font-semibold">{escrowData.receivers.length} addresses</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-white/60 text-sm">Verify escrow data on Base Sepolia Explorer</p>
          </div>
